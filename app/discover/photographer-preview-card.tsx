		...
		<div className="my-2 grid gap-5 rounded-md bg-white py-5 md:grid-cols-[21rem_1fr] md:gap-2 shadow-lg pr-1">
			<ScrollArea className="h-full w-full rounded-md md:col-start-2">
				<div className="flex w-max gap-1">
					{shuffle(assets).slice(0, 7).map((asset, idx) => (
						<div
							key={idx}
							className="relative mr-1 aspect-[3/2] h-full w-48 flex-shrink-0 overflow-hidden border shadow-md sm:w-60 md:w-80 lg:w-[28rem]"
							style={{
								position: 'relative',
							}}
						>
					...
					{assets.length > 7 && (
						<div className="relative mr-1 aspect-[3/2] h-full w-48 flex-shrink-0 overflow-hidden rounded-md border shadow-md sm:w-60 md:w-80 lg:w-[28rem]">
							<Dialog>
								<DialogTrigger className="text-md absolute left-1/2 top-1/2 z-10 m-2 -translate-x-1/2 -translate-y-1/2 transform rounded bg-white px-3 py-1 font-medium text-black shadow-md hover-gradient-bg">
									View all
								</DialogTrigger>
								<PhotographerOverlay photographer={photographer} account={account} assets={assets} />
							</Dialog>
							<div
								className="absolute left-0 top-0 h-full w-full bg-cover bg-center"
								style={{
