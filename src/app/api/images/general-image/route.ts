...
export async function POST(request: Request) {
  ...
  const uploadPromises = files.map(
    async (file: { filename: string; contentType: string; parts: number }) => {
      const Key = `client-galleries/${folderName}/${file.filename}`;
      ...
      try {
        const { UploadId } = await s3Client.send(
          new CreateMultipartUploadCommand({
            ...
            Metadata: {
              skills: JSON.stringify([])  // Removed 'x-amz-meta-' prefix
            }
          })
        );
        ...
        console.error(
          `Error Initializing multipart upload for ${file.filename}:`,
          error
        );
 ...
 * Returns the key, CloudFront URL, size, and skills of each image.
 */
export async function GET(request: Request) {
  ...
  try {
    const listCommand = new ListObjectsV2Command({
      Bucket: process.env.AWS_BUCKET_NAME!,
      Prefix: `client-galleries/${folderId}`,
    });
    ...
    const images = await Promise.all(
      listResponse.Contents.map(async (object) => {
        if (object.Key) {
          ...
          // Parse the skills from the metadata
          let skills: string[] = [];
          if (headResponse.Metadata && headResponse.Metadata['x-amz-meta-skills']) {  // Keep 'x-amz-meta-' prefix here
            try {
              skills = JSON.parse(headResponse.Metadata['x-amz-meta-skills']);  // Keep 'x-amz-meta-' prefix here
            } catch (error) {
              console.error(`Error parsing skills for ${object.Key}:`, error);
            }
          }
